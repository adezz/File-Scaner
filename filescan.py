...
class FileScan(object):
    ...
    def _data(self, item):
        ...
        if rule_true_status:
            ...
            logger.warning(
                '[FileScan] url: {} 正在进行二次验证是否存在信息泄露'.format(rule_true))
            # 如果验证失败 说明存在漏洞
            if not rule_false_status:
                self.result.put(rule_true)
                logger.warning(
                    '[FileScan Done] url: {} 存在信息泄露'.format(rule_true))
    ...
    def _check(self, url, result):
        ...
        if response != None:
            ...
            if status_code:
                check.append(self._status_code(
    ...
    def _reg(self, content, result):
        ...
        if isinstance(result, list):
            for num, reg in enumerate(result):
                if len(re.findall(reg.lower(), content.decode('utf8').lower())) > 0:
                    return True
        return False
    ...
    def _black_list(self, response):
        ...
        if _black:
            ...
            if black_html:
                for num, tstr in enumerate(black_html):
                    if len(re.findall(tstr.lower(), response.content.lower())) > 0:
                        return True
        return False
    ...
    def _data_print(self):
        ...
        with open(str(int(time.time())) + '.txt', 'a+', encoding='utf8') as f:
            while self.result.empty() == False:
                f.write(str(self.result.get()) + '\n')
    ...
    def main2(self, txt):
        logging.info("正在测试txt :{}".format(txt))
        start_time = int(time.time())

        # url的读取处理
        _ = list(set([x.replace('\n', '') if x.startswith('http') else 'http://' +
                      x.replace('\n', '') for x in open(txt, 'r', encoding='utf8').readlines()]))
