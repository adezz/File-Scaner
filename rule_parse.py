...
class Rule(object):
    ...
    def _white_list(self, whitelist):
        for item in whitelist:
            ...
            if _is_suffix:
                ...
                for num, tstr in enumerate(suffix):
                    for x, y in enumerate(_name):
                        ...
                        for num, file in enumerate(_rule):
                            ...
                            self.dir_list.append({
                                "rule_true": "{}.{}".format(file, tstr),
                                "rule_false": "{}.{}".format(y.get("rule_false", ""), tstr),
                                "result": _result
                            })
            else:
                for x, y in enumerate(_name):
    ...
    def _url_parse(self):
        ...
        if len(_path) > 2:
            ...
            if "." in _path[-1]:
                _path.pop()
            for num, tstr in enumerate(_path):
                # 假如子元素为空
                if not tstr:
                    continue
                _dir = _dir+"/"+tstr
    ...
    def _replace(self, tstr):
        ...
        if "[TIME]" not in tstr:
            _r = self._url_parse()
            result = [tstr.replace("[DOMAIN]", _r.get("domain", "")).replace(
                "[HOST]", _r.get("host", "")).replace("[HOSTNAME]", _r.get("hostname", "ms"))]
        else:
            # 替换时间格式
            ...
            for x in range(0, self.timenum):
                ...
                strf_list = [
                    tstr.replace("[TIME]", timedel.strftime('%Y-%m-%d')),
                    tstr.replace("[TIME]", timedel.strftime('%Y%m%d')),
                    tstr.replace("[TIME]", timedel.strftime('%Y_%m_%d'))
                ]
        ...
        for n, u in enumerate(_url_list):
            for num, rule in enumerate(self.dir_list):
                ...
                for x, _rule in enumerate(replace_rule_true):
                    ...
                    _dict["rule_false"] = "{}/{}".format(
                        u, rule.get("rule_false", ""))
